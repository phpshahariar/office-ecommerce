...
//        $this->productValidation($request);
    ...
    public function unpublishedProduct($id){
        ...
        $product->status = 2;
        $product->update();
        return redirect('/shopper-dash')->with('message', 'Product Info Unpublished Successfully !!!');
    }
    public function publishedProduct($id){
        $product = Product::where('id',$id)->first();
        $product->status = 1;
        $product->update();
        return redirect('/shopper-dash')->with('message', 'Product Info Published Successfully !!!');
    }
