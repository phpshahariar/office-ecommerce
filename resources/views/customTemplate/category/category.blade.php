...
@section('mainContent')
        ...
        <div class="home_content d-flex flex-column align-items-center justify-content-center">
            @if(Session::get('lang') == 'বাংলা')
                @if(isset($category_name->category_name))
                    <h2 class="home_title">{{$category_name->category_name_ban}}</h2>
                @endif
                @if(isset($category_name->sub_category_name))
                    <h2 class="home_title">{{$category_name->sub_category_name_ban}}</h2>
                @endif
                @if(isset($brand_name->brand_name))
                    <h2 class="home_title">{{$brand_name->brand_name_ban}}</h2>
                @endif
            @elseif(Session::get('lang') == 'नहीं')
                @if(isset($category_name->category_name))
                    <h2 class="home_title">{{$category_name->category_name_hin}}</h2>
                @endif
                @if(isset($category_name->sub_category_name))
                    <h2 class="home_title">{{$category_name->sub_category_name_hin}}</h2>
                @endif
                @if(isset($brand_name->brand_name))
                    <h2 class="home_title">{{$brand_name->brand_name_hin}}</h2>
                @endif
            @else
                @if(isset($category_name->category_name))
    ...
    <div class="shop">
        <div class="container">
            <div class="row">
                @include('customTemplate.category.include.side_content')
                @include('customTemplate.category.include.main_content')
            </div>
        </div>
    </div>
